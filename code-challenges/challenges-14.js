...
const LastWord = (str) => {
    let num = str.lastIndexOf(" ")+1;
    let newStr = "";
    for(let i=0 ; i<=num;i++) newStr = str.slice(i);
    return newStr;
}
...
const letterCounter = (str) => {
  ...
  newStr.forEach(word => {
     ...
     for(let i=0 ; i<word.length ; i++){
           ...
           newChar.push(`${word[i]}${counter}`); 
           counter = 1;

     }
     newWord.push(newChar.toString().replaceAll(',',''));
  })
