...
const oddFiltration = (arr) => arr.filter(item => item%2)
...
// 
// ------------------------
const vowelsFiltration = (arr) => arr.filter(item => !/(?:a|e|i|o|u)/.test(item));
...
const skillComparison = (arr1, arr2) => arr1.filter(item => !arr2.includes(item)).concat(arr2.filter(item => !arr1.includes(item)));
